    ...
    const fetchCartItems = async () => {
      try {
        ...
        if (data.success) {
          ...
          // Step 2: Fetch product details for each productId
          const detailedProducts = await Promise.all(
            productsInCart.map(async (item) => {
              ...
              return { ...item, ...productData.product }; // Assuming the response contains a `product` object
            })
          );

          setCartItems(detailedProducts); // Set the products with full details
        } else {
